...
Item {
    ...
    // This used to be a Process running `cat` on a 1s repeating Timer. The file
    // only changes when matugen reruns -- and matugen-apply.sh calls reload.sh
    // afterwards, which reloads the whole shell anyway -- so the poll re-read an
    // unchanged file forever. MatugenColors is instantiated in twenty places, so
    // that was twenty forks a second, every second, for colours that had not
    // moved. (Worse when /tmp/qs_colors.json is absent, as it is after a reboot
    // until matugen next runs: twenty failing forks a second instead.)
    //
    // FileView reads it inside quickshell, once, and watches it after that.
    FileView {
